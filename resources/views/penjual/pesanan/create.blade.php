    ...
    <div class="card shadow-sm p-4">
        <div class="row mb-3">
            ...
            <div class="col-md-6 text-md-end">
                ...
                <p>Status Pembayaran: 
                    <span class="badge bg-{{ $order->status == 'complete' ? 'success' : ($order->status == 'pending' ? 'warning' : 'danger') }}">
        ...
        <form action="{{ route('penjual.pesanan.updateStatus', $order->id) }}" method="POST" class="mt-4">
            ...
            <div class="form-group">
                ...
                <div class="btn-group" role="group" aria-label="Status pengiriman">
                    @php
                        $pengirimanStatus = ['dikemas' => 'Dikemas', 'dikirim' => 'Dikirim', 'diterima' => 'Diterima', 'belum_diterima' => 'Belum Diterima'];
                    @endphp
                    @foreach ($pengirimanStatus as $value => $label)
                        <input type="radio" class="btn-check" name="status_pesanan" id="status-{{ $value }}" value="{{ $value }}"
                            autocomplete="off" {{ old('status_pesanan', $order->status_pesanan) === $value ? 'checked' : '' }}>
