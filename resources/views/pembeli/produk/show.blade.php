...
@section('content')
    ...
    @if ($produk->harga_setelah_diskon < $produk->harga)
        ...
        <del class="text-color">
            Rp{{ number_format($produk->harga, 0, ',', '.') }}
            <span class="badge bg-danger ms-2">{{ $diskon }}% OFF</span>
        </del>
        <br>
        <ins class="fs-4 fw-bold text-warning">
    ...
    @endif
                ...
                <div class="mt-2">
                    ...
                    <p class="text-color">{{ $produk->user->email ?? '-' }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mt-5 text-color">
        <h5 class="mb-3 text-dark">Produk Lain dari Toko Ini</h5>
        <div class="row">
                    ...
                    <div class="card-box shadow border-0 rounded h-100">
                        <div class="position-relative" style="height: 200px; overflow: hidden;">
                            @if ($item->gambar)
                                <img src="{{ asset('storage/' . $item->gambar) }}" class="w-100 h-100" style="object-fit: cover;"
                                    alt="{{ $item->nama }}">
                            @else
                                <img src="{{ asset('vendors/images/no-image.png') }}" class="w-100 h-100" style="object-fit: cover;"
                        ...
                        <div class="p-3">
                            ...
                            <a href="{{ route('pembeli.produk.show', $item->id) }}"
                                class="btn btn-outline-primary mt-2 btn-sm w-100">Lihat Produk</a>
                        </div>
                    </div>
                </div>
            @empty
